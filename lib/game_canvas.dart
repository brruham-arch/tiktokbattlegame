...
class GamePainter extends CustomPainter {
  ...
  void _drawBackground(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(size.width - 60, 20, 16, 16),
      moonPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(size.width - 56, 24, 8, 8),
  ...
  void _drawGround(Canvas canvas, Size size) {
    ...
    // Ground blocks (pixel style)
    final groundColors = [
      const Color(0xFF4CAF50),
      const Color(0xFF388E3C),
    ];
    final dirtColor = const Color(0xFF795548);

    // Top grass layer
    for (int i = 0; i < (size.width / s).ceil() + 1; i++) {
      final color = groundColors[i % 2];
      canvas.drawRect(
        Rect.fromLTWH(i * s, engine.groundY, s, s),
        Paint()..color = color,
      );
    }
    ...
    for (int row = 1; row < 4; row++) {
      for (int i = 0; i < (size.width / s).ceil() + 1; i++) {
        canvas.drawRect(
          Rect.fromLTWH(i * s, engine.groundY + row * s, s, s),
    ...
    // Ground outline
    canvas.drawLine(
      Offset(0, engine.groundY),
      Offset(size.width, engine.groundY),
      Paint()
        ..color = Colors.black
        ..strokeWidth = 1,
    );
  ...
  void _drawFloatingTexts(Canvas canvas) {
    for (final ft in engine.floatingTexts) {
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(ft.x, ft.y));
    }
  }
